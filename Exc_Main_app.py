...
Армирование сечения - симметричное с привязкой ЦЕНТРА стержня $a$.
Расчет огнестойкости выполняется по упрощенной методике, представленной в СП 468.1325800.2019.
При расчете огнестойкости рассматривается прогрев с одной стороны.''')
st.subheader('ОБЯЗАТЕЛЬНО ОТКЛЮЧИТЕ АВТОПЕРЕВОД!')
